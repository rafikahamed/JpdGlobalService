...
public interface FileDetailsRepository extends CrudRepository<DataConsole, Long>{
	 ...
	 @Query("SELECT t FROM DataConsole t where datename(qq,sale_date) = :quater and  t.reportIndicator = :fileType and t.userCode = :userCode") 
	 ...
	 @Query(value="SELECT t FROM DataConsole t where datename(qq,sale_date) = :quater and t.userCode = :userCode") 
